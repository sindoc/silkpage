...
<xsl:template match="autolayout">
  ...
  <xsl:if test="$collect.xref.targets != 'only' or
		$collect.xref.targets = 'only'">
    <xsl:if test="$generate.rdf = 'yes'">
  ...
  <xsl:if test="$collect.xref.targets = 'yes' or
		$collect.xref.targets = 'only'">
    <xsl:if test="$generate.translation = 'yes'">
      <xsl:for-each select="config[@param=$trl.label]">
	<xsl:apply-templates select="../toc|../notoc" mode="trl">
	  <xsl:with-param name="lang" select="@value"/>
	</xsl:apply-templates>
      </xsl:for-each>
    </xsl:if>
  </xsl:if>
