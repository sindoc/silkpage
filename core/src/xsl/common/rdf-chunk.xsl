...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		exclude-result-prefixes=" html rdf cvs sweb xweb "
                version='1.0'>
...
<xsl:template match="toc|tocentry|notoc" mode="rdf">
  <xsl:call-template name="rdf.tocentry"/>
  <xsl:apply-templates select="tocentry" mode="rdf"/>
</xsl:template>
...
<xsl:template name="rdf.tocentry">
  ...
  <xsl:choose>
	...
	<xsl:apply-templates select="$webpage/webpage" mode="rdf"/>
      </xsl:variable>
